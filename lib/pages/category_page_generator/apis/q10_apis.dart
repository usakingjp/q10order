...
class Q10Apis {
  ...
  Future<Map<String, dynamic>> getAllGoodsInfo(
    ...
    try {
      do {
        ...
        Map<String, dynamic> responsJson = convert.jsonDecode(response.body);
        if (responsJson["ResultCode"] == 0) {
          // respons["ResultObject"]["Items"] as List<Map<String, dynamic>>;
          result["Items"].addAll([
            for (var e in responsJson["ResultObject"]["Items"])
              GetAllGoodsInfoModel.fromMap(e as Map<String, dynamic>)
          ]);
          if (page != null && page.isNotEmpty) {
            break;
          }
          if (int.parse(responsJson["ResultObject"]["TotalPages"].toString()) >
              int.parse(
                  responsJson["ResultObject"]["PresentPage"].toString())) {
            body["Page"] = (pageIndex + 1).toString();
          } else {
            break;
          }
        } else {
          result["ResultCode"] = responsJson["ResultCode"];
          result["ResultMsg"] = responsJson["ResultMsg"];
          break;
        }
      } while (true);
      return result;
  ...
  Future<Map<String, dynamic>> getItemDetaiInfo(
    ...
    try {
      ...
      Map<String, dynamic> responsJson = convert.jsonDecode(response.body);
      if (responsJson["ResultCode"] == 0) {
        return responsJson["ResultObject"][0] as Map<String, dynamic>;
      } else {
        return {
          'error': '${responsJson["ResultCode"]} : ${responsJson["ResultMsg"]}'
        };
      }
    } catch (e) {
      print(e.toString());
